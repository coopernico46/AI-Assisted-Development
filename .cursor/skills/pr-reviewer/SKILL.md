...
- **MUST rules** from `.cursor/rules/rules.mdc` for the touched area — e.g. for API specs: `expect(Schema.parse(body)).toBeTruthy()` exactly; `z.strictObject()` not `z.object()`; **2+ API calls each in its own `test.step()`**; one tag per test; URLs/paths/messages from `config`/`enums` not hardcoded; state-mutating tests have cleanup hooks.
...
```bash
.cursor/skills/pr-reviewer/scripts/scan-constitution.sh "$BASE"
```
