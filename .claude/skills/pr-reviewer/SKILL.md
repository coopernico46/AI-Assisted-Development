...
The changed file paths decide which rules apply. This is the heart of doing it *right* — an API branch and a UI branch are judged by different skills. Map every changed path to its skills and **read those skill files** before reviewing. Always also hold the `CLAUDE.md` Constitution.
