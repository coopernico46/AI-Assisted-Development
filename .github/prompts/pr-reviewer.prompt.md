...
**Missing env ≠ branch defect.** Specs read runtime tokens a `*.setup.ts` mints from real credentials. No env → run dies at auth. Report verbatim as *"could not run — missing env tokens (`<var>`); run manually before merge"*. NEVER invent/hardcode/stub a token to force green.
...
Ask *"Want me to implement any of these findings?"* User picks subset. Match sibling patterns, minimal/on-scope, re-run Phase 4.
